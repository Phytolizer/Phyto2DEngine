...
void phyto::engine::Texture::loadData(const GLubyte* data)
{
	bind();
...
phyto::engine::Texture::Texture(GLsizei width, GLsizei height, const GLubyte* data) : m_width{width}, m_height{height}
{
	glGenTextures(1, &m_id);
	glBindTexture(GL_TEXTURE_2D, m_id);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_width, m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
